...
SDL3.Render.Texture? converted_texture = null;
...
public int main (string[] args) {
    SDL3.Init.set_app_metadata ("SDL3 Vala Renderer Example 17 - Read Pixels", "1.0",
    ...
    success = SDL3.Render.create_window_and_renderer ("SDL3 Vala Renderer Example 17 - Read Pixels",
    ...
    while (is_running) {
        ...
        SDL3.Surface.Surface? surface_read = null;
        SDL3.Surface.Surface? converted_surface = null;
        ...
        SDL3.Render.render_clear (renderer);
        {
            ...
            surface_read = SDL3.Render.render_read_pixels (renderer, null);
            // This is also expensive, but easier: convert the pixels to a format we want.
            if (surface_read != null && (surface_read.format != SDL3.Pixels.PixelFormat.RGBA8888)
                && (surface_read.format != SDL3.Pixels.PixelFormat.BGRA8888)) {
                converted_surface = SDL3.Surface.convert_surface (surface_read, SDL3.Pixels.PixelFormat.RGBA8888);
            } else {
                converted_surface = SDL3.Surface.create_surface (texture_width, texture_height, SDL3.Pixels.PixelFormat.RGBA8888);
                SDL3.Surface.blit_surface (surface_read, null, converted_surface, null);
            }
            SDL3.Surface.destroy_surface (surface_read);
            ...
            if (converted_surface != null) {
                ...
                if ((converted_surface.w != converted_texture_width) || (converted_surface.h != converted_texture_height)) {
                    SDL3.Render.destroy_texture (converted_texture);
                    converted_texture = SDL3.Render.create_texture (
                                                                    renderer,
                    ...
                    if (converted_texture == null) {
                        SDL3.Log.log ("Couldn't (re)create conversion texture: %s", SDL3.Error.get_error ());
                        return 1;
                    }
            ...
            SDL3.Render.update_texture (converted_texture, null, converted_surface.pixels, converted_surface.pitch);
            SDL3.Surface.destroy_surface (converted_surface);
            ...
            SDL3.Render.render_texture (renderer, converted_texture, null, dst_rect);
        }
